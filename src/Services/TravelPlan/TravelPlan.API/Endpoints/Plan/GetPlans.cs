...
namespace TravelPlan.API.Endpoints.Plan
{
    ...
    public class GetPlans : ICarterModule
    {
        ...
        {
            app.MapGet("/plans", async ([AsParameters] PaginationRequest request, ISender sender, IHttpContextAccessor httpContext) =>
            {
                var userId = httpContext.HttpContext!.GetUserIdFromJwt();

                var result = await sender.Send(new GetPlansQuery(userId, request));
